...
const { cookieExpires, useI18n, projectId } = config

export const TOKEN_KEY = projectId + 'Token'

export const LOCAL = projectId + 'Local'
...
export const setTagNavListInLocalstorage = list => {
  localStorage[projectId + 'TagNaveList'] = JSON.stringify(list)
}
/**
 * @returns {Array} 其中的每个元素只包含路由原信息中的name, path, meta三项
 */
export const getTagNavListFromLocalstorage = () => {
  const list = localStorage[projectId + 'TagNaveList']
 ...
 * @param {*} pid
 */
export const treeDataTranslate = (data, menuIdList, id = 'id', pid = 'parentId') => {
  let res = []
  let temp = {}
  let parentIds = []
 ...
 * @param totalCount 删除后总数
 * @returns {boolean}
 */
export const checkDelete = (page, pageSize, totalCount) => {
  return totalCount <= pageSize * (page - 1) && page !== 1 ? --page : page
}
